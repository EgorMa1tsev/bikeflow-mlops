...
├── data/                   датасет и журнал прогнозов — создаются командами, в git не хранятся
...
```bash
python -m bikeflow.replay --evening-boost 1.6 --boost-from 2018-11-01
```

Модель не знает об акции, поэтому её ошибка на ноябрьских вечерах вырастает с 243 до 584, а в
остальные часы не меняется. Такой рост ошибки при неизменных входах — это concept drift.
